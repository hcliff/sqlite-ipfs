...
Module["onRuntimeInitialized"] = function onRuntimeInitialized() {
 ...
 var sqlite3_create_module_v2 = cwrap("sqlite3_create_module_v2", "number", [ "number", "string", "number", "number", "number" ]);
 ...
 var sqlite3_malloc = cwrap("sqlite3_malloc", "number", [ "number" ]);
 ...
 Statement.prototype["bind_"] = Statement.prototype.bind;
 ...
 Statement.prototype.getBlob = function getBlob(pos) {
  ...
  for (var i = 0; i < size; i += 1) {
   result[i] = SAFE_HEAP_LOAD(ptr + i, 1, 0);
  }
  return result;
 };
 ...
 StatementIterator.prototype["next"] = function next() {
  ...
  try {
   this.db.handleError(sqlite3_prepare_v2_sqlptr(this.db.db, this.nextSqlPtr, -1, apiTemp, pzTail));
 ...
 Database.prototype["export"] = function exportDatabase() {
  ...
  var binaryDb = FS.readFile(this.filename, {
   encoding: "binary"
  });
 ...
 function extract_blob(ptr) {
  ...
  for (var j = 0; j < size; j += 1) {
   blob_arg[j] = SAFE_HEAP_LOAD(blob_ptr + j, 1, 0);
  }
  return blob_arg;
 }
 ...
 Database.prototype.create_vtab = function create_vtab(cons) {
  ...
  const sqlite3_module = _malloc(Object.keys(module_things).length * 4);
  let i = 0;
...
function _utimes(path, times) {
 ...
 if (times) {
  var mtime = times + 8;
  time = (SAFE_HEAP_LOAD(mtime | 0, 4, 0) | 0) * 1e3;
  time += (SAFE_HEAP_LOAD(mtime + 4 | 0, 4, 0) | 0) / 1e3;
 } else {
  time = Date.now();
 }
